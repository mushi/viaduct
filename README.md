...
## What
A complete Terraform project deploying a VLESS+Reality proxy ([Xray-core](https://github.com/XTLS/Xray-core)) and a [Psiphon Conduit](https://github.com/Psiphon-Inc/conduit) relay (donating bandwidth to Psiphon users) on a single low-cost host, with metrics sent to Grafana Cloud. The total cost is limited to the monthly cost of the Hetzner host (includes 20 TB traffic monthly).

### VLESS
End users connect using a client app - e.g. V2RayNG (Android), v2rayN (Windows), or Nekoray - that routes their device's traffic through the server. It works like a VPN for the user, though the underlying protocol is a proxy. This project generates connection URIs per user.
