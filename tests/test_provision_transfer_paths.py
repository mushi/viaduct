...
class DownloadLoopStdinTest(unittest.TestCase):
    """The per-user backup loop reads its file list on FD 3, not stdin. download() runs
    ssh (via remote cat), and ssh with an inherited stdin reads the rest of the piped
    list itself — draining it so the loop stops after the first file and only the first
    client's backup is saved. This behaviourally reproduces both patterns."""

    HARNESS = (
        'set -e\n'
        'dl() { cat >/dev/null; }\n'          # ssh-like: consumes whatever stdin it has
        'n=0\n'
        '{PATTERN}\n'
        'echo "PROCESSED=$n"\n'
    )
    ...
    def _run(self, pattern: str) -> str:
        return subprocess.run(
            ["bash", "-c", self.HARNESS.replace("{PATTERN}", pattern)],
            capture_output=True, text=True, timeout=30, stdin=subprocess.DEVNULL).stdout

    def test_fd3_pattern_processes_every_entry(self):
        out = self._run(
            'while IFS= read -r rf <&3; do n=$((n+1)); dl; done '
            '3< <(printf "a\\nb\\nc\\nd\\n")')
        self.assertIn("PROCESSED=4", out,
                      "the FD-3 loop did not process all four entries")

    def test_the_naive_stdin_pattern_would_drain(self):
        """Proves the harness is real: the old `done < <(...)` form drops all but one."""
        out = self._run(
            'while IFS= read -r rf; do n=$((n+1)); dl; done '
            '< <(printf "a\\nb\\nc\\nd\\n")')
        self.assertIn("PROCESSED=1", out,
                      "the naive stdin loop unexpectedly survived — the anchor is invalid")

    def test_provision_uses_the_fd3_form(self):
        body = PROVISION.read_text()
        loop = body[body.index("Per-user .uuid"):]
        loop = loop[: loop.index("Provisioning complete")]
        self.assertIn("read -r rf <&3", loop,
                      "the per-user download loop no longer reads on FD 3")
        self.assertIn("done 3<", loop,
                      "the per-user download loop no longer feeds the list on FD 3")
